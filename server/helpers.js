...
async function gitClone(settings) {
  ...
  try {
    console.log(
      `downloading repository from https://github.com/${userName}/${repo}.git`
    );
...
async function buildStart(buildObject) {
  ...
  axios
    .post('/build/start', startBuild)
    .catch(e => console.error(e.code, 'build start'));
  return buildObject;
}
...
async function buildCancel(buildObject) {
  ...
  axios
    .post('/build/cancel', buildId)
    .catche(e => console.error(e.code, 'build cancel'));
  return buildObject;
}
